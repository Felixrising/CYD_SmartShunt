/**
 * @file lv_conf.h
 * CYD Smart Shunt – LVGL 9 configuration
 *
 * Based on the ESP32-Cheap-Yellow-Display (CYD) reference:
 *   reference/ESP32-Cheap-Yellow-Display/Examples/LVGL9/lv_conf.h
 *
 * This project uses a custom TFT_eSPI flush (LV_USE_TFT_ESPI=0) and no demos.
 * For LVGL 8, use the template in reference/.../Examples/LVGL8/lv_conf.h
 * (different option names, e.g. LV_MEM_CUSTOM vs LV_USE_STDLIB_*).
 *
 * With LV_CONF_INCLUDE_SIMPLE, place this file in a directory on the include
 * path (e.g. project include/) so LVGL finds it and does not use internal defaults.
 */
...
#define LV_FONT_MONTSERRAT_20 1   /* value-edit digits ~50% larger than default 14 */
...
/*Interface for TFT_eSPI - we use custom display flush, so keep 0 */
